...
{
  private final String file = "stakeholder.ser";
  
 //These are the classes used to write to the files
  FileWriter fileWriter;
  PrintWriter printWriter;
  
  //The classes used to read objects from the serialized file
  FileInputStream fis;
...
//reading the customer objects from the serialized file and adding to the arraylist
  private ArrayList<Customer>customersList()
  {
      ...
      //checking to see if an arrayList is not empty
      if(!customers.isEmpty())
      {
          //sorting the arrayList in ascending order according to the StHolderId
...
private void writeCustomerOutFile()
{
    ...
    try
    {
      ...
      {
          printWriter.printf(placeholder, customersList().get(i).getStHolderId(), customersList().get(i).getFirstName(), customersList().get(i).getSurName(), formatDate(customersList().get(i).getDateOfBirth()), calculateAge(customersList().get(i).getDateOfBirth()));
      }
...
//reading the supplier objects from the serialized file and add to arrayList
...
private void writeSupplierOutFile()
{
   ...
   {
     printWriter.print(header);
     printWriter.printf(placeholder, "ID", "Name", "Prod Type", "Description");
     printWriter.print(separator);
     
     for (int i = 0; i < suppliersList().size(); i++)
     {
         printWriter.printf(placeholder, suppliersList().get(i).getStHolderId(),  suppliersList().get(i).getName(),  suppliersList().get(i).getProductType(),  suppliersList().get(i).getProductDescription());
     }
...
public void closeFile(String stakeholderfile)
{
    try
    {
     fileWriter.close();
     printWriter.close();
     System.out.println(stakeholderfile + "has been closed");
    ...
    {
        ReadSerializedFile s = new ReadSerializedFile();
